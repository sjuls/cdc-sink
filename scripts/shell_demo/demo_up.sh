...
echo ">>> Create sneaky view"
cockroach sql --insecure --port=${SRC_SQL_PORT} -e="CREATE VIEW movr.sneaky_view AS SELECT * FROM movr.rides;"
...
cockroach sql --insecure --echo-sql --port="$SINK_SQL_PORT" -e="CREATE SCHEMA movr.cdcsink AUTHORIZATION root;"
cockroach sql --insecure --echo-sql --port="$SINK_SQL_PORT" -e="GRANT ALL PRIVILEGES ON SCHEMA movr.cdcsink TO cdcsink;"
...
cockroach sql --insecure --echo-sql --port="$SINK_SQL_PORT" -e="GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA movr.cdcsink TO cdcsink;"
cockroach sql --insecure --echo-sql --port="$SINK_SQL_PORT" -e="ALTER TABLE movr.cdcsink.leases OWNER TO root;"
cockroach sql --insecure --echo-sql --port="$SINK_SQL_PORT" -e="REVOKE UPDATE ON TABLE movr.cdcsink.leases FROM cdcsink;"

echo ">>> Waiting ..."
sleep 30

cockroach sql --insecure --echo-sql --port="$SINK_SQL_PORT" -e="GRANT INSERT, UPDATE ON TABLE movr.cdcsink.leases TO cdcsink;"
